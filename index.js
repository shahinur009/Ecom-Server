...
async function run() {
  try {
    const userCollections = client.db("rab_Baby_Zone").collection("users");
    const productCollections = client
      .db("rab_Baby_Zone")
      .collection("products");
    const bannerCollections = client.db("rab_Baby_Zone").collection("banners");
...
app.listen(port, () => {
  console.log(`RAB Baby Zone server is running on ${port}`);
});
